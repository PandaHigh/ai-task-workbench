...
describe("PluginManager", () => {
  ...
  it("should show empty state when no plugins", async () => {
    mockCall.mockResolvedValue([]);
    await act(async () => { render(<PluginManager />); });
    expect(screen.getByText("暂无 MCP Server 插件")).toBeInTheDocument();
  });

  it("should display plugin names", async () => {
    mockCall.mockResolvedValue([
      { id: "p1", name: "filesystem", description: "File system MCP", type: "mcp-server", config: { name: "filesystem", command: "npx", args: ["-y", "@mcp/server"] }, status: "stopped", enabled: false },
    ]);
    await act(async () => { render(<PluginManager />); });
