...
function TypewriterText({ text, speed = 20 }: { text: string; speed?: number }) {
  ...
  const html = useMemo(() => {
    try {
      return marked.parse(displayed, { async: false }) as string;
    } catch {
      return displayed;
    }
  }, [displayed]);
...
export function TaskWizard() {
  ...
  useEffect(() => {
    Promise.resolve(call("config.get", { key: "defaultTimeout" }))
      .then((res) => {
        const v = (res as Record<string, unknown>)?.value;
        if (typeof v === "number") setTimeoutMinutes(v);
      })
      .catch(() => {});
  }, [call]);
      ...
      <div className="flex-1 flex flex-col overflow-hidden" style={pageEnterStyle()}>
        ...
        {mode === "quick" ? (
          ...
          <>
            ...
            {step === 0 && (
              <div
                className="flex-1 flex items-center justify-center max-md:pb-16"
                role="tabpanel"
                id="wizard-panel-0"
                aria-labelledby="wizard-step-0"
                style={{ animation: "fadeIn 0.4s ease-out" }}
              >
                {isStartingSession ? (
                  <div className="text-center">
                    <div
                      className="w-48 h-24 mx-auto rounded-lg mb-3"
                      style={{ background: "var(--bg-tertiary)", animation: "pulse 1.5s ease-in-out infinite" }}
                    />
                  ...
                  <div className="text-center px-4 max-w-md w-full">
                    ...
                    <div className="glass-card p-4 mb-4 text-left">
                      ...
                      <p className="text-sm mb-3 font-mono" style={{ color: "var(--green)" }}>
                        {defaultDir()}
                      </p>
                      <button
                    ...
                    <div className="glass-card p-4 mb-4 text-left">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          style={{ color: "var(--blue)", background: "rgba(77, 107, 254, 0.1)" }}
                        >
                          浏览
                        </button>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={confirmDir}
                          className="px-4 py-2 rounded text-xs font-semibold shrink-0"
                          style={{ background: "var(--green)", color: "#fff" }}
                        >
                          确认
                        </button>
            ...
            {step === 1 && (
              <div
                className="flex-1 flex flex-col overflow-hidden max-md:pb-16"
                role="tabpanel"
                id="wizard-panel-1"
                aria-labelledby="wizard-step-1"
              ...
              >
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  {messages.map((msg, i) => {
                    ...
                    const mdHtml =
                      isAssistant && !isTypingTarget ? (marked.parse(msg.content, { async: false }) as string) : "";
                    return (
                      <div
                      ...
                      >
                        <div
                          className={`max-w-[80%] px-3 py-2 rounded-lg text-xs ${isAssistant ? "glass-card-sm" : ""}`}
                          style={{
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 flex flex-col">
                      ...
                      <div className="flex items-center justify-between mt-1">
                        {inputError ? (
                          <p id="chat-error" className="text-xs" style={{ color: "var(--red)" }} role="alert">
                            {inputError}
                          </p>
                        ) : (
            ...
            {step === 2 && taskParams && (
              <div
                className="flex-1 overflow-y-auto p-6 max-md:p-4 max-md:pb-20"
                role="tabpanel"
                id="wizard-panel-2"
                aria-labelledby="wizard-step-2"
                style={{ animation: "fadeIn 0.3s ease-out" }}
              >
                <div className="glass-card p-4" style={{ animation: "slideUp 0.35s ease-out" }}>
                  ...
                  <div className="space-y-3 text-xs">
                    ...
                    {/* Editable content */}
                    <div>
                      <label className="block mb-1 font-bold" style={{ color: "var(--text-secondary)" }}>
                        任务内容
                      </label>
                      <textarea
                        value={editedContent}
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    disabled={creating}
                    className="px-6 py-3 rounded text-sm font-semibold disabled:opacity-50"
