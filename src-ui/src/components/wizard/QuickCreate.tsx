...
export function QuickCreate() {
  ...
  const handleCreate = async (autoStart: boolean) => {
    ...
    if (!content.trim()) {
      setContentError("请填写任务描述");
      return;
    }
    if (content.trim().length < 2) {
      setContentError("任务描述至少 2 个字符");
      return;
    }
    if (creating) return;
        ...
        <div className="glass-card p-4">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setIsDefaultDir(false);
              if (dirError) validateDir(e.target.value);
            }}
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              if (contentError && e.target.value.trim().length >= 2) setContentError("");
            }}
