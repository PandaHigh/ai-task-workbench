  ...
  return (
    <div className="space-y-1.5">
      {workers.map((w) => {
        ...
        return (
          <div key={w.taskId} className="rounded p-2" style={{ background: "var(--bg-tertiary)", borderLeft: `2px solid ${color}` }}>
            <div className="flex items-center gap-2 mb-1">
              <span className="inline-block w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: color }} />
              <span className="text-xs font-bold" style={{ color }}>{w.roleName}</span>
