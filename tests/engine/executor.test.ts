...
describe("Executor injectInstructions", () => {
  let queueManager: MockQueueManager;
  let notifications: { method: string; params: Record<string, unknown> }[];
  let notify: (method: string, params: Record<string, unknown>) => void;
  ...
  it("should include injected instructions in buildSystemPrompt", async () => {
    const { Executor } = await import("../../src-engine/src/engine/executor.js");
    const executor = new Executor(queueManager as unknown as ConstructorParameters<typeof Executor>[0], notify, "run-1");

    executor.injectInstructions("Focus on error handling");
    executor.injectInstructions("Use TypeScript strict mode");

    const task: Partial<TaskDefinition> = { content: "Test" };
    const context: TaskContext = {
      workingDir: "/tmp", goals: [], terminationConditions: [],
      lastTenCommits: [], nextFiveTasks: [], lessonsLearned: [],
    };
    const prompt = (executor as unknown as { buildSystemPrompt: (t: Partial<TaskDefinition>, c: TaskContext) => string }).buildSystemPrompt(task, context);

    expect(prompt).toContain("USER INJECTED INSTRUCTIONS");
    expect(prompt).toContain("Focus on error handling");
    expect(prompt).toContain("Use TypeScript strict mode");
  });

  it("should drain injected instructions after first buildSystemPrompt call", async () => {
    const { Executor } = await import("../../src-engine/src/engine/executor.js");
    const executor = new Executor(queueManager as unknown as ConstructorParameters<typeof Executor>[0], notify, "run-1");

    executor.injectInstructions("One-time instruction");
    ...
    const buildFn = (executor as unknown as { buildSystemPrompt: (t: Partial<TaskDefinition>, c: TaskContext) => string }).buildSystemPrompt.bind(executor);

    const prompt1 = buildFn(task, context);
    expect(prompt1).toContain("One-time instruction");

    const prompt2 = buildFn(task, context);
    expect(prompt2).not.toContain("One-time instruction");
    expect(prompt2).not.toContain("USER INJECTED");
  });

  it("should log when instructions are injected", async () => {
    const { Executor } = await import("../../src-engine/src/engine/executor.js");
    const executor = new Executor(queueManager as unknown as ConstructorParameters<typeof Executor>[0], notify, "run-1");

    executor.injectInstructions("New direction for the project");
    ...
    const logCall = mockStore.appendLog.mock.calls[mockStore.appendLog.mock.calls.length - 1];
    expect(logCall[1].message).toContain("Instructions injected");
    expect(logCall[1].message).toContain("New direction for the project");
  });

  it("should include injected instructions alongside other context data", async () => {
    const { Executor } = await import("../../src-engine/src/engine/executor.js");
    const executor = new Executor(queueManager as unknown as ConstructorParameters<typeof Executor>[0], notify, "run-1");

    executor.injectInstructions("Prioritize performance");

    const task: Partial<TaskDefinition> = { content: "Test" };
    const context: TaskContext = {
      workingDir: "/tmp", goals: ["g1"], terminationConditions: ["done"],
      lastTenCommits: [{ hash: "abc1234", message: "init commit", timestamp: 1000, isAiCommit: true }],
      nextFiveTasks: [], lessonsLearned: [],
    };
    const prompt = (executor as unknown as { buildSystemPrompt: (t: Partial<TaskDefinition>, c: TaskContext) => string }).buildSystemPrompt(task, context);

    expect(prompt).toContain("USER INJECTED INSTRUCTIONS");
    expect(prompt).toContain("Prioritize performance");
    expect(prompt).toContain("init commit");
  });
});
